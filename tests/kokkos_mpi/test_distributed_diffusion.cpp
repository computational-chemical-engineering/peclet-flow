// cfd-gpu — distributed (MPI) Kokkos grid-halo integration: the foundational pattern for the
// multi-rank Kokkos sdflow.
//
// Solves a periodic implicit-diffusion system ((idiag)I - beta*Lap) u = b by Red-Black
// Gauss-Seidel, with the field decomposed over MPI ranks (ORB) and the per-colour ghost exchange
// done by transport-core's portable GPU-resident halo (GridHalo) instead of a single-process
// periodic fill. The cfd CCField and peclet::core::View<double> are the SAME Kokkos type
// (x-fastest, default memory space), so the solver field exchanges directly. The distributed RB-GS
// is algebraically identical to the single-rank sweep (the halo supplies the same neighbour values
// + the GLOBAL red-black parity via the block's global origin), so the multi-rank result must equal
// a serial reference to machine precision. Validates the consumption pattern (replace fillGhosts
// with halo.exchange) the full sdflow MPI port will follow. Runs on any backend.
#include <mpi.h>
...
int main(int argc, char** argv) {
  ...
  int fail = 0, size = 1, rank = 0;
  {
    ...
    for (int k = 0; k < SWEEPS; ++k)
      for (int color = 0; color < 2; ++color) {
        dev.exchange(
            u);  // cross-rank + periodic ghost exchange (replaces the single-process fillGhosts)
    ...
    for (int z = 0; z < gs[2]; ++z)
      for (int y = 0; y < gs[1]; ++y)
        for (int x = 0; x < gs[0]; ++x)
          hbr((long)(x + G) + (long)(y + G) * ge.x + (long)(z + G) * (long)ge.x * ge.y) =
              source(x, y, z, gs);
    Kokkos::deep_copy(br, hbr);
    Kokkos::deep_copy(ur, 0.0);
    for (int k = 0; k < SWEEPS; ++k)
      for (int color = 0; color < 2; ++color) {
        periodicFill(ur, ge);
        diffSmoothColor(ur, br, ge, geo, G, BETA, AC, color, empty);
      }
    auto hur = Kokkos::create_mirror_view(ur);
    Kokkos::deep_copy(hur, ur);
    ...
    idx.forEachInner([&](const IVec<kDim>& l) {
      ...
      const double r =
          hur((long)(gx + G) + (long)(gy + G) * ge.x + (long)(gz + G) * (long)ge.x * ge.y);
    ...
    if (maxdiff > 1e-12) {
      ++fail;
      std::fprintf(stderr, "[rank %d] max|distributed - serial| = %.3e\n", rank, maxdiff);
    }
  }
  ...
  if (rank == 0) {
    if (totalFail == 0)
      std::printf("OK (np=%d): distributed RB-GS diffusion == serial reference\n", size);
    else
