// cfd-gpu — the PRODUCTION CutcellMG, multi-rank. Folds the distributed halo + reductions into the
// real class (gated behind PECLET_FLOW_MPI) rather than a test-only copy.
//
// Solves the periodic cut-cell pressure Poisson by the real CutcellMG::solvePCG (MG-PCG: CG
// preconditioned by one symmetric V-cycle) two ways: distributed (initMpi over MPI_COMM_WORLD) and
// single-rank (init() on the full grid, the validated single-GPU path). Every rank runs the
// single-rank reference redundantly and compares its own block. This proves CutcellMG.initMpi + the
// gated fill()->exchange + dot/maxabs/removeMean->Allreduce produce the same field as the
// single-GPU class, and (np=1) that the gating is byte-identical. Tolerance: np=1 is exact; np>1
// agrees to ~1e-8 -- the MG-PCG is ADAPTIVE, so the inner-product Allreduce (summed in a different
// order than the single-rank local sum) steers the Krylov path slightly differently and, with the
// float-stored operator (residual floor ~1e-7 relative), the two converged solutions differ at
// ~1e-9. That is an operation-order/roundoff difference, not a method change (the deterministic
// V-cycle machinery is bit-tight to 1e-11 -- see test_distributed_mg). Build with -DCFD_MPI.
#include <mpi.h>
...
static void fillSource(CCField b, C3 ext, C3 og, IVec<3> gs) {
  ...
  for (std::size_t c = 0; c < b.extent(0); ++c)
    hb(c) = 0.0;
...
int main(int argc, char** argv) {
  ...
  int fail = 0, size = 1, rank = 0;
  {
    ...
    CutcellMG mg;
    mg.initMpi(gs[0], gs[1], gs[2], NLEV, MPI_COMM_WORLD);
    CutcellMG::Level& l0 = mg.level(0);
    auto hx = setupAndSolve(mg, l0.ext, l0.og);

    // single-rank reference (the validated single-GPU path) on the full grid; every rank, compare
    // its block
    CutcellMG ref;
    ...
    // np=1 (no Allreduce reordering) is bit-exact; np>1 is the float-op MG-PCG reduction-order
    // floor (~1e-9).
    const double tol = (size == 1) ? 1e-13 : 1e-8;
    if (maxdiff > tol) {
      ++fail;
      std::fprintf(stderr, "[rank %d] max|distributed - single-rank CutcellMG| = %.3e (tol %.0e)\n",
                   rank, maxdiff, tol);
    } else if (rank == 0)
      std::printf("  max|distributed - single-rank| = %.3e (np=%d, tol %.0e)\n", maxdiff, size,
                  tol);
  }
  int totalFail = 0;
  ...
  if (rank == 0) {
    if (totalFail == 0)
      std::printf("OK (np=%d): production CutcellMG MG-PCG distributed == single-rank\n", size);
    else
