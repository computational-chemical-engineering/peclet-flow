...
//
// The third MPI milestone: the geometric V-cycle itself, decomposed over ranks. A level hierarchy
// is built by coarsening the global grid 2:1 per level; each level gets its OWN transport-core halo
// (GridHalo over a BlockDecomposer of that level's grid). The transport-core ORB decomposition
// coarsens cleanly -- each rank's coarse block is exactly its fine block halved -- so the
// restriction / prolongation stay LOCAL (a coarse cell's 2^3 fine children all live on the same
// rank); only the smoother / residual / prolong ghosts cross ranks, via the per-level halo. The
// singular periodic null space is removed with the distributed mean (MPI_Allreduce). Running the
// same code with MPI_COMM_SELF (a size-1 block = the full grid) is the serial reference: the
// distributed V-cycle must match it to ~1e-11. Validates the per-level- halo + local-transfer
// pattern that turns the single-GPU CutcellMG into the multi-rank pressure multigrid.
#include <mpi.h>
...
#include "mac_cutcell_mg.hpp"  // restrictAvg, prolongAdd, residualCutcell, buildCutcellOp, cutcellSmoothColor
...
// Distributed multigrid for the periodic all-fluid Laplacian; same code for MPI_COMM_WORLD
// (decomposed) and MPI_COMM_SELF (size-1 full-grid serial reference).
struct DistMG {
  MPI_Comm comm;
  int rank, size;
  std::vector<Level> lv;
  ...
  void init(MPI_Comm c, IVec<kDim> gs) {
    ...
    for (int L = 0; L < NLEV; ++L) {
      ...
      v.dev = std::make_unique<GridHalo<double>>();
      v.dev->init(*v.halo);
      const auto& idx = v.halo->indexer();
      const auto eg = idx.sizeInclGhost(), ino = idx.sizeInner(), o = idx.originInclGhost();
      v.e = {(int)eg[0], (int)eg[1], (int)eg[2]};
      v.inner = {(int)ino[0], (int)ino[1], (int)ino[2]};
      v.og = {(int)o[0], (int)o[1], (int)o[2]};
      v.cfac = {(int)cf[0], (int)cf[1], (int)cf[2]};
      v.n = idx.numCellsInclGhost();
      v.gCells = (long)g[0] * g[1] * g[2];
      v.x = CCField("x", v.n);
      v.rhs = CCField("rhs", v.n);
      v.res = CCField("res", v.n);
      v.ox = CCField("ox", v.n);
      v.oy = CCField("oy", v.n);
      v.oz = CCField("oz", v.n);
      Kokkos::deep_copy(v.ox, 1.0);
      Kokkos::deep_copy(v.oy, 1.0);
      Kokkos::deep_copy(v.oz, 1.0);  // all-fluid
      for (FPV* p : {&v.AC, &v.AW, &v.AE, &v.AS, &v.AN, &v.AB, &v.AT})
        *p = FPV("A", v.n);
      const double s = 1.0 / (double)(cf[0]);  // uniform: cfac isotropic here
      peclet::flow::buildCutcellOp(v.AC, v.AW, v.AE, v.AS, v.AN, v.AB, v.AT, CCConst(v.ox),
                                   CCConst(v.oy), CCConst(v.oz), v.e, G, 1.0 / (cf[0] * cf[0]),
                                   1.0 / (cf[1] * cf[1]), 1.0 / (cf[2] * cf[2]));
      (void)s;
  ...
  void removeMean(Level& v, CCField f) {
    const double mean = meanSum(v, f) / (double)v.gCells;
    CCExec sp;
    C3 e = v.e;
    CCField ff = f;
    Kokkos::parallel_for(
  ...
  void smooth(Level& v, int sweeps) {
    for (int k = 0; k < sweeps; ++k)
      for (int color = 0; color < 2; ++color) {
        v.dev->exchange(v.x);
        peclet::flow::cutcellSmoothColor(v.x, CCConst(v.rhs), FPC(v.AC), FPC(v.AW), FPC(v.AE),
                                         FPC(v.AS), FPC(v.AN), FPC(v.AB), FPC(v.AT), v.e, v.og, G,
                                         color);
      }
  }
  void vcycle(int L) {
  ...
  // solve A x = b by NVCYC V-cycles; b set into level-0 rhs (mean removed). Solution left in
  // level-0 x.
  void solve(CCField b) {
    Level& l0 = lv[0];
    Kokkos::deep_copy(l0.rhs, b);
    removeMean(l0, l0.rhs);
    Kokkos::deep_copy(l0.x, 0.0);
    for (int v = 0; v < NVCYC; ++v)
      vcycle(0);
  }
};
...
int main(int argc, char** argv) {
  ...
  int fail = 0, size = 1, rank = 0;
  {
    ...
    DistMG mg;
    mg.init(MPI_COMM_WORLD, gs);
    Level& l0 = mg.lv[0];
    CCField b("b", l0.n);
    {
      auto hb = Kokkos::create_mirror_view(b);
      for (Index c = 0; c < l0.n; ++c)
        hb(c) = 0.0;
      mg.lv[0].halo->indexer().forEachInner([&](const IVec<kDim>& l) {
        hb(mg.lv[0].halo->indexer().localMdToLocal(l)) =
            source(l[0] + l0.og.x, l[1] + l0.og.y, l[2] + l0.og.z, gs);
      });
    ...
    DistMG ref;
    ref.init(MPI_COMM_SELF, gs);
    Level& r0 = ref.lv[0];
    CCField rb("rb", r0.n);
    {
      auto hrb = Kokkos::create_mirror_view(rb);
      for (Index c = 0; c < r0.n; ++c)
        hrb(c) = 0.0;
      ref.lv[0].halo->indexer().forEachInner([&](const IVec<kDim>& l) {
        hrb(ref.lv[0].halo->indexer().localMdToLocal(l)) =
            source(l[0] + r0.og.x, l[1] + r0.og.y, l[2] + r0.og.z, gs);
      });
      Kokkos::deep_copy(rb, hrb);
    }
    ref.solve(rb);
    auto hr = Kokkos::create_mirror_view(r0.x);
    Kokkos::deep_copy(hr, r0.x);
    ...
    if (maxdiff > 1e-11) {
      ++fail;
      std::fprintf(stderr, "[rank %d] max|distributed - serial V-cycle| = %.3e\n", rank, maxdiff);
    }
  }
  int totalFail = 0;
  ...
  if (rank == 0) {
    if (totalFail == 0)
      std::printf("OK (np=%d): distributed cut-cell MG V-cycle == serial reference\n", size);
    else
