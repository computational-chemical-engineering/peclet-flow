...
using peclet::core::halo::GridHalo; using peclet::core::halo::GridHaloTopology;
using peclet::flow::CCField; using peclet::flow::CCConst; using peclet::flow::CCExec; using peclet::flow::C3; using peclet::flow::FPV; using peclet::flow::FPC;
...
struct DistMG {
  ...
  void init(MPI_Comm c, IVec<kDim> gs) {
    ...
    for (int L = 0; L < NLEV; ++L) {
      ...
      peclet::flow::buildCutcellOp(v.AC, v.AW, v.AE, v.AS, v.AN, v.AB, v.AT, CCConst(v.ox), CCConst(v.oy), CCConst(v.oz),
  ...
  void smooth(Level& v, int sweeps) {
    for (int k = 0; k < sweeps; ++k)
      for (int color = 0; color < 2; ++color) {
        v.dev->exchange(v.x);
        peclet::flow::cutcellSmoothColor(v.x, CCConst(v.rhs), FPC(v.AC), FPC(v.AW), FPC(v.AE), FPC(v.AS), FPC(v.AN),
  ...
  void vcycle(int L) {
    ...
    smooth(v, PRE);
    v.dev->exchange(v.x);
    peclet::flow::residualCutcell(v.res, CCConst(v.x), CCConst(v.rhs), FPC(v.AC), FPC(v.AW), FPC(v.AE), FPC(v.AS),
                          FPC(v.AN), FPC(v.AB), FPC(v.AT), v.e, G);
    Level& c = lv[L + 1];
    peclet::flow::restrictAvg(c.rhs, CCConst(v.res), c.e, v.e, G, c.inner, v.ratio);  // LOCAL (coarse block = fine/2)
