...
//
// Folds the distributed halo into the real velocity-MG (gated behind PECLET_FLOW_MPI). VelocityMG
// has NO global reductions (the backward-Euler velocity operator is non-singular -> no mean
// removal, and the V-cycle is a fixed iteration, not a Krylov method), so the fold is purely
// fill()->per-level halo exchange + the block- origin red-black parity -- and the distributed
// V-cycle is therefore BIT-tight to the single-rank one (no Allreduce reordering at all). Solves an
// all-fluid periodic backward-Euler diffusion (fine stencil = idiag*I - beta*Lap; staircase coarse
// op with theta=1, no pin, full clean-fluid mask) distributed (initMpi) vs single-rank (init() on
// the full grid). Build with -DCFD_MPI.
#include <mpi.h>
...
int main(int argc, char** argv) {
  ...
  int fail = 0, size = 1, rank = 0;
  {
    ...
    auto setupAndSolve = [&](VelocityMG& vmg, C3 ext, C3 og) {
      ...
      for (FPV* p : {&AW, &AE, &AS, &AN, &AB, &AT})
        Kokkos::deep_copy(*p, (float)(-BETA));
      ...
      Kokkos::deep_copy(th, 1.0);
      Kokkos::deep_copy(solid, 0.0);
      Kokkos::deep_copy(clean, 1.0);
    ...
    VelocityMG vmg;
    vmg.initMpi(gs[0], gs[1], gs[2], NLEV, MPI_COMM_WORLD);
    ...
    if (maxdiff > 1e-11) {
      ++fail;
      std::fprintf(stderr, "[rank %d] max|distributed - single-rank VelocityMG| = %.3e\n", rank,
                   maxdiff);
    } else if (rank == 0)
      std::printf("  max|distributed - single-rank| = %.3e (np=%d)\n", maxdiff, size);
  }
  int totalFail = 0;
  MPI_Allreduce(&fail, &totalFail, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
  if (rank == 0) {
    if (totalFail == 0)
      std::printf("OK (np=%d): production VelocityMG V-cycle distributed == single-rank\n", size);
    else
