...
// centerToFace, the projected face field is halo-exchanged, the cut-cell pressure MG is MPI-folded.
// The superficial velocity <u> (hence the permeability k = mu*<u>/F) is reduced over ranks and must
// equal single-rank: np=1 bit-exact, np>1 to the MG-PCG reduction-order floor (the inner-product
// Allreduce reorders the Krylov path). Build with -DCFD_MPI.
#include <mpi.h>
