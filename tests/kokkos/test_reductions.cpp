// Correctness of the Kokkos MAC reductions (peclet::flow::localSumMax / localDot / subtractAll)
// over the inner cells of an extended (inner+ghost) block, vs a host reference. Validates the
// parallel_reduce indexing (x-fastest, +ghost) and the mean-subtraction. Runs on whatever backend
// Kokkos was built for (CUDA locally; OpenMP for CI). The MPI_Allreduce wrapper is host MPI and
// unchanged.
#include <cmath>
#include <cstdio>
...
int main(int argc, char** argv) {
  ...
  int status = 0;
  {
    ...
    if (!status)
      std::printf(
          "[mac_reductions] PASS: sum/max/dot + mean-subtract match host (%ld inner cells, exec: "
          "%s)\n",
          count, Exec::name());
  }
  Kokkos::finalize();
