...
int main(int argc, char** argv) {
  ...
  int status = 0;
  {
    ...
    auto upd = [&](const char* nm, std::vector<double>& h) {
      CCField v(nm, n);
    ...
    auto upf = [&](const char* nm, std::vector<float>& h) {
      Kokkos::View<float*, CCMem> v(nm, n);
      auto m = Kokkos::create_mirror_view(v);
      for (std::size_t i = 0; i < n; ++i)
        m(i) = h[i];
      Kokkos::deep_copy(v, m);
    ...
    auto AC = upf("AC", hAC), AW = upf("AW", hAW), AE = upf("AE", hAE), AS = upf("AS", hAS),
         AN = upf("AN", hAN), AB = upf("AB", hAB), AT = upf("AT", hAT);
      ...
      for (int z = g; z < e.z - g; ++z)
        for (int y = g; y < e.y - g; ++y)
          for (int xx = g; xx < e.x - g; ++xx) {
            if (((og.x + xx + og.y + y + og.z + z) & 1) != color)
              continue;
            ...
            double ac = hAC[i];
            if (std::fabs(ac) < 1e-30)
              continue;
            double s = (double)hAE[i] * rx[i + sx] + (double)hAW[i] * rx[i - sx] +
                       (double)hAN[i] * rx[i + sy] + (double)hAS[i] * rx[i - sy] +
                       (double)hAT[i] * rx[i + sz] + (double)hAB[i] * rx[i - sz];
            rx[i] = (hb[i] - s) / ac;
          }
    auto close = [](double a, double b2) {
      return std::fabs(a - b2) <= 1e-9 * (1.0 + std::fabs(b2));
    };
    int bad = 0;
    for (std::size_t i = 0; i < n; ++i)
      if (!close(gx[i], rx[i]))
        ++bad;
    if (bad) {
      std::fprintf(stderr, "FAIL: %d RB-GS cells differ\n", bad);
      status = 1;
    }

    // Laplacian convergence: AC=6, A_off=-1, no mask -> 6x - sum_nbr = b ; residual drops.
    std::vector<float> lc(n, 6.f), lo(n, -1.f);
    auto LC = upf("LC", lc), LW = upf("LW", lo), LE = upf("LE", lo), LS = upf("LS", lo),
         LN = upf("LN", lo), LB = upf("LB", lo), LT = upf("LT", lo);
    CCField px = upd("px", hx);
    CCConst noMask;  // empty mask
    auto resid = [&]() {
      auto m = getd(px);
      double r = 0;
      for (int z = g; z < e.z - g; ++z)
    ...
    for (int it = 0; it < 100; ++it)
      ibmRbgsSweep(px, b, LC, LW, LE, LS, LN, LB, LT, noMask, e, og, g);
    double r1 = resid();
    if (!(r1 < 0.2 * r0)) {
      std::fprintf(stderr, "FAIL: IBM smoother residual %.3e -> %.3e\n", r0, r1);
      status = 1;
    }
    ...
    Off3 off{0, 0, 0};
    ibmVolfrac(th, sf, e, off);
    auto gth = getd(th);
    for (std::size_t i = 0; i < n; ++i)
      if (!close(gth[i], 1.0))
        ++bad;
    ibmVolfrac(th, so, e, off);
    auto gth2 = getd(th);
    for (std::size_t i = 0; i < n; ++i)
      if (!close(gth2[i], 0.0))
        ++bad;
    ibmSolidMask(mk, sf, e, off);
    auto gmk = getd(mk);
    for (std::size_t i = 0; i < n; ++i)
      if (!close(gmk[i], 0.0))
        ++bad;
    ibmSolidMask(mk, so, e, off);
    auto gmk2 = getd(mk);
    for (std::size_t i = 0; i < n; ++i)
      if (!close(gmk2[i], 1.0))
        ++bad;
    if (bad && !status) {
      std::fprintf(stderr, "FAIL: volfrac/mask property\n");
      status = 1;
    }

    if (!status)
      std::printf(
          "[mac_ibm] PASS: variable-coeff RB-GS matches host; Laplacian resid %.2e -> %.2e; "
          "volfrac/mask ok (exec: %s)\n",
          r0, r1, CCExec::name());
  }
  Kokkos::finalize();
  return status;
