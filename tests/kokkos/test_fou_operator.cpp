...
int main(int argc, char** argv) {
  ...
  int status = 0;
  {
    ...
    const int comp = 1;
    Kokkos::parallel_for(
        "fou_op", Kokkos::RangePolicy<Kokkos::DefaultExecutionSpace>(0, ninner),
        KOKKOS_LAMBDA(long c) {
          ...
          fou_operator_aniso(comp, x, y, z, Ua, Va, Wa, dt, 1.0, 1.0, 1.0, aC, axm, axp, aym, ayp,
                             azm, azp);
          long sx = 1, sy = ex, sz = (long)ex * ey, i = (long)x + (long)y * ex + (long)z * sz;
          double lhs = cC * Pa(x, y, z) + cxm * P(i - sx) + cxp * P(i + sx) + cym * P(i - sy) +
                       cyp * P(i + sy) + czm * P(i - sz) + czp * P(i + sz);
          double rhs = dt * advect_fou(comp, x, y, z, Ua, Va, Wa, Pa);
          double anisoDiff = Kokkos::fabs(aC - cC) + Kokkos::fabs(axm - cxm) +
                             Kokkos::fabs(axp - cxp) + Kokkos::fabs(aym - cym) +
                             Kokkos::fabs(ayp - cyp) + Kokkos::fabs(azm - czm) +
                             Kokkos::fabs(azp - czp);
          // cC - sum|off| = dt*div(advecting velocity) (exact identity); >= 0 only for a div-free
          // field (as in the real solver). With random velocity here it can be negative — not a
          // defect.
          resid(c) = Kokkos::fabs(lhs - rhs) + anisoDiff;
        });
    auto hr = Kokkos::create_mirror_view(resid);
    Kokkos::deep_copy(hr, resid);
    int bad = 0;
    double maxr = 0;
    for (long c = 0; c < ninner; ++c) {
      if (hr(c) > 1e-12 * (1 + std::fabs(hr(c))))
        ++bad;
      if (hr(c) > maxr)
        maxr = hr(c);
    }
    if (bad) {
      std::fprintf(stderr, "FAIL: %ld/%ld cells operator != dt*advect_fou (max %.2e)\n", (long)bad,
                   ninner, maxr);
      status = 1;
    }
    if (!status)
      std::printf(
          "[fou_operator] PASS: operator == dt*advect_fou and aniso(s=1)==fou_operator (%ld cells, "
          "exec %s)\n",
