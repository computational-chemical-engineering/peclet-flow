...
int main(int argc, char** argv) {
  ...
  int status = 0;
  {
    ...
    // neighbours (cut cells) form, exercising the Robust-Scaled cut-cell no-slip (not just
    // masking).
    ...
    // channel SDF: sdf(iy) = min(iy - ylo, yhi - iy) (>0 in fluid, <0 in the wall) at each ext
    // cell.
    CCField sdf("sdf", n);
    {
      ...
      for (int z = 0; z < e.z; ++z)
        for (int y = 0; y < e.y; ++y)
          for (int x = 0; x < e.x; ++x) {
            int iy = y - g;
            double s = std::min((double)iy - ylo, yhi - (double)iy);
    ...
    int nCut =
        buildIbmOverlay<1>(CCConst(sdf), e, g, offU, /*bc_type Dirichlet*/ 0, ov, idMap, counter);
      ...
      Kokkos::parallel_for(
          ...
          KOKKOS_LAMBDA(int y, int z) {
            ...
            for (int gl = 0; gl < g; ++gl) {
              uu(base + gl) = uu(base + gl + Nx);
              uu(base + g + Nx + gl) = uu(base + g + gl);
            }
          });
      Kokkos::parallel_for(
          ...
          KOKKOS_LAMBDA(int x, int y) {
            ...
            for (int gl = 0; gl < g; ++gl) {
              uu(base + (long)gl * sz) = uu(base + (long)(gl + Nz) * sz);
              uu(base + (long)(g + Nz + gl) * sz) = uu(base + (long)(g + gl) * sz);
            }
          });
      space.fence();
    };
    ...
    for (int step = 0; step < 600; ++step) {
      // RHS b' = rscale * (idiag*u + F)   (Robust-Scaled; inhom=0 for no-slip walls)
      {
        CCField uu = u, bb = b;
        auto rs = rscale;
        double id = idiag, ff = F;
        Kokkos::parallel_for(
            "rhs", Kokkos::RangePolicy<CCExec>(space, 0, n),
            KOKKOS_LAMBDA(std::size_t i) { bb(i) = rs(i) * (id * uu(i) + ff); });
        space.fence();
      }
      for (int it = 0; it < 200; ++it) {
        fillXZ();
        ibmRbgsStencilColor(u, CCConst(b), MConst(AC), MConst(AW), MConst(AE), MConst(AS),
                            MConst(AN), MConst(AB), MConst(AT), CCConst(mask), e, C3{0, 0, 0}, g,
                            0);
        fillXZ();
        ibmRbgsStencilColor(u, CCConst(b), MConst(AC), MConst(AW), MConst(AE), MConst(AS),
                            MConst(AN), MConst(AB), MConst(AT), CCConst(mask), e, C3{0, 0, 0}, g,
    ...
    for (int iy = 0; iy < ny; ++iy) {
      ...
      if (iy > ylo && iy < yhi) {  // fluid interior
        double yy = iy;
        double ue = (F / (2.0 * mu)) * (yy - ylo) * (yhi - yy);
        l2num += (un - ue) * (un - ue);
        l2den += ue * ue;
      }
    }
    ...
    std::printf(
        "[poiseuille_ibm] cut cells=%d; U_max=%.5f analytic=%.5f (err %.2e); profile L2 err=%.2e\n",
        nCut, umax, Uana, umaxErr, l2err);
    if (nCut <= 0) {
      std::fprintf(stderr, "FAIL: no cut cells found\n");
      status = 1;
    }
    if (umaxErr > 3e-2) {
      std::fprintf(stderr, "FAIL: U_max off analytic\n");
      status = 1;
    }
    if (l2err > 3e-2) {
    ...
    if (!status)
      std::printf("[poiseuille_ibm] PASS: IBM cut-cell channel reproduces Poiseuille (exec %s)\n",
