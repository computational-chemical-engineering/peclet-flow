...
//     z-uniform mode). We check the amplitude ratio, that div(u) stays ~0 (projection), and that
//     the spatial profile is preserved (uniform decay, no mode corruption).
...
static void initTG(FlowReference& s, std::vector<double>& u0, std::vector<double>& v0) {
  ...
  Kokkos::deep_copy(hu, s.u());
  Kokkos::deep_copy(hv, s.v());
  Kokkos::deep_copy(hw, s.w());
  u0.assign((size_t)e.x * e.y * e.z, 0.0);
  v0.assign(u0.size(), 0.0);
  ...
  Kokkos::deep_copy(s.u(), hu);
  Kokkos::deep_copy(s.v(), hv);
  Kokkos::deep_copy(s.w(), hw);
}
...
int main(int argc, char** argv) {
  ...
  int status = 0;
  {
    ...
    // --- STOKES (advection off): tight check vs the discrete decay ---
    {
      ...
      std::printf(
          "[sdflow_tg] STOKES: ratio=%.6f expected=%.6f (rel err %.2e); max|div|=%.2e; profile "
          "err=%.2e\n",
          ratio, fExpect, rateErr, divmax, profErr);
      if (rateErr > 5e-3) {
        std::fprintf(stderr, "FAIL: decay rate off\n");
        status = 1;
      }
      if (divmax > 1e-9) {
        std::fprintf(stderr, "FAIL: divergence not zero\n");
        status = 1;
      }
      if (profErr > 5e-3) {
        std::fprintf(stderr, "FAIL: profile not preserved\n");
        status = 1;
      }
    }

    // --- NS (advection on): decay should stay close to the same diffusion rate ---
    // Post-projection divergence = the pressure-Poisson residual. Compare the wired multigrid
    // pressure solve (a few V-cycles) against the old plain RB-GS (80 sweeps): MG should drive div
    // far lower for much less work.
    {
      FlowReference sMg(N, nu, dt), sRb(N, nu, dt);
      sMg.setAdvection(true);
      sMg.setPressureMultigrid(true, 6);
      sRb.setAdvection(true);
      sRb.setPressureMultigrid(false, 0);
      sRb.setIterations(300, 80);
      std::vector<double> u0, v0;
      initTG(sMg, u0, v0);
      initTG(sRb, u0, v0);
      ...
      const double rateErr = std::fabs(ratio - fExpect) / fExpect;
      std::printf(
          "[sdflow_tg] NS:     ratio=%.6f expected=%.6f (rel err %.2e); max|div| MG(6 V-cyc)=%.2e "
          "vs RB-GS(80 sweeps)=%.2e\n",
          ratio, fExpect, rateErr, divMg, divRb);
      if (rateErr > 5e-2) {
        std::fprintf(stderr, "FAIL: NS decay too far from TG\n");
        status = 1;
      }
      if (divMg > 1e-7) {
        std::fprintf(stderr, "FAIL: MG divergence too large\n");
        status = 1;
      }
      if (!(divMg < divRb)) {
        std::fprintf(stderr, "FAIL: MG did not beat RB-GS\n");
        status = 1;
      }
    }
