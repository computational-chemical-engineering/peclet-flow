...
// ====================================================== gate C2: the interface Courant BAND
// (WO-R2)
//
...
void gateZalesakBand() {
  ...
  for (int k = 0; k < 2; ++k) {
    ...
    std::printf(
        "  wispEps %-6g : band Courant after step 1 %.4f, after %ld %.4f, worst %.4f, "
        "volume drift %.3e\n",
  ...
  std::printf(
      "  a-priori: the disk's own faces reach %.4f (r = 0.40) and the band's outermost "
