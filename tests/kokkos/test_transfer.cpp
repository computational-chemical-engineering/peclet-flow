// Correctness of the Kokkos multigrid transfer (peclet::flow::restrict_ / prolong) + projection
// correction (peclet::flow::correct) vs host replication. Plus property checks: restriction of a
// constant is that constant; prolongation of a constant adds that constant. Runs on whatever
// backend Kokkos has.
#include <cmath>
#include <cstdio>
...
int main(int argc, char** argv) {
  ...
  int status = 0;
  {
    ...
    for (auto* vec : {&hfine, &hu, &hv, &hw, &hphi})
      for (auto& x : *vec)
        x = uf(rng);
    for (auto& x : hcoarse)
      x = uf(rng);
    ...
    long lc = [&](int x, int y, int z) {
      return (long)x + (long)y * cext.x + (long)z * (long)cext.x * cext.y;
    }(0, 0, 0);
    (void)lc;
    auto CI = [&](int x, int y, int z) {
      return (std::size_t)x + (std::size_t)y * cext.x +
             (std::size_t)z * (std::size_t)cext.x * cext.y;
    };
    auto FI = [&](int x, int y, int z) {
      return (std::size_t)x + (std::size_t)y * fext.x +
             (std::size_t)z * (std::size_t)fext.x * fext.y;
    };
    int bad = 0;
    ...
    for (int icz = 0; icz < cinner.z; ++icz)
      for (int icy = 0; icy < cinner.y; ++icy)
        for (int icx = 0; icx < cinner.x; ++icx) {
          double s = 0;
          for (int dz = 0; dz < ratio.z; ++dz)
            for (int dy = 0; dy < ratio.y; ++dy)
              for (int dx = 0; dx < ratio.x; ++dx)
                s += hfine[FI(ratio.x * icx + dx + g, ratio.y * icy + dy + g,
                              ratio.z * icz + dz + g)];
          double r = s / (ratio.x * ratio.y * ratio.z);
          if (!close(gco[CI(icx + g, icy + g, icz + g)], r))
            ++bad;
        }
    ...
    auto trilerp_h = [&](double x, double y, double z) {
      ...
      double c00 = F(x0, y0, z0) * (1 - wx) + F(x0 + 1, y0, z0) * wx,
             c10 = F(x0, y0 + 1, z0) * (1 - wx) + F(x0 + 1, y0 + 1, z0) * wx;
      double c01 = F(x0, y0, z0 + 1) * (1 - wx) + F(x0 + 1, y0, z0 + 1) * wx,
             c11 = F(x0, y0 + 1, z0 + 1) * (1 - wx) + F(x0 + 1, y0 + 1, z0 + 1) * wx;
      double c0 = c00 * (1 - wy) + c10 * wy, c1 = c01 * (1 - wy) + c11 * wy;
      return c0 * (1 - wz) + c1 * wz;
    };
    for (int ifz = 0; ifz < finner.z; ++ifz)
      for (int ify = 0; ify < finner.y; ++ify)
        for (int ifx = 0; ifx < finner.x; ++ifx) {
          ...
          if (!close(gf[FI(ifx + g, ify + g, ifz + g)], r))
            ++bad;
        }

    // --- correction ---
    TField u = upN("u", hu, fn), v = upN("v", hv, fn), w = upN("w", hw, fn),
           phi = upN("phi", hphi, fn);
    correct(u, v, w, phi, fext, g);
    auto gu = get(u, fn), gvv = get(v, fn), gw = get(w, fn);
    for (int z = g; z < fext.z - g; ++z)
      for (int y = g; y < fext.y - g; ++y)
        for (int x = g; x < fext.x - g; ++x) {
          std::size_t i = FI(x, y, z);
          long sx = 1, sy = fext.x, sz = (long)fext.x * fext.y;
          if (!close(gu[i], hu[i] - (hphi[i] - hphi[i - sx])))
            ++bad;
          if (!close(gvv[i], hv[i] - (hphi[i] - hphi[i - sy])))
            ++bad;
          if (!close(gw[i], hw[i] - (hphi[i] - hphi[i - sz])))
            ++bad;
        }
    if (bad) {
      std::fprintf(stderr, "FAIL: %d transfer/correct cells differ\n", bad);
      status = 1;
    }
    ...
    for (int icz = 0; icz < cinner.z; ++icz)
      for (int icy = 0; icy < cinner.y; ++icy)
        for (int icx = 0; icx < cinner.x; ++icx)
          if (!close(gcc[CI(icx + g, icy + g, icz + g)], 3.5))
            ++bad;
    ...
    for (int ifz = 0; ifz < finner.z; ++ifz)
      for (int ify = 0; ify < finner.y; ++ify)
        for (int ifx = 0; ifx < finner.x; ++ifx)
          if (!close(gff[FI(ifx + g, ify + g, ifz + g)], 2.0))
            ++bad;
    if (bad && !status) {
      std::fprintf(stderr, "FAIL: constant-field property\n");
      status = 1;
    }

    if (!status)
      std::printf(
          "[mac_transfer] PASS: restrict/prolong/correct match host + constant properties (exec: "
          "%s)\n",
          TExec::name());
  }
  Kokkos::finalize();
