...
int main(int argc, char** argv) {
  ...
  int status = 0;
  {
    ...
    auto up = [&](const char* nm, std::vector<double>& h) {
      DView v(nm, n); auto m = Kokkos::create_mirror_view(v);
    ...
    auto hT = Kokkos::create_mirror_view(outTVD); Kokkos::deep_copy(hT, outTVD);
    auto hF = Kokkos::create_mirror_view(outFOU); Kokkos::deep_copy(hF, outFOU);

    // host reference
    int bad = 0;
    HostAcc Uh{hU.data(), ex, ey}, Vh{hV.data(), ex, ey}, Wh{hW.data(), ex, ey}, Ph{hP.data(), ex, ey};
    ...
    for (long c = 0; c < ninner; ++c) {
      ...
      if (!close(hT(c), rT) || !close(hF(c), rF)) ++bad;
    }
    if (bad) { std::fprintf(stderr, "FAIL: %d/%ld advection cells differ\n", bad, ninner); status = 1; }
    else std::printf("[advection] PASS: %ld cells, Koren-TVD + FOU match host (exec: %s)\n",
