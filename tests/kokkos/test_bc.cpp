// Correctness of the Kokkos domain BC ghost fills (peclet::flow::bcVelocityComp / bcOutflowComp /
...
static void hostVel(std::vector<double>& f, B3 ext, int g, int a, int s, int comp, double wall,
                    int fold) {
  ...
  for (int p0 = 0; p0 < dims[b]; ++p0)
    for (int p1 = 0; p1 < dims[c]; ++p1) {
...
int main(int argc, char** argv) {
  ...
  int status = 0;
  {
    ...
    auto close = [](double x, double y) {
      return std::fabs(x - y) <= 1e-12 * (1.0 + std::fabs(y));
    };
    ...
    if (bad) {
      std::fprintf(stderr, "FAIL: %d mismatches across %d BC cases\n", bad, cases);
      status = 1;
    } else
      std::printf(
          "[mac_bc] PASS: %d BC cases (velocity normal/tangential/fold + wall openness) match host "
          "(exec: %s)\n",
          cases, BExec::name());
  }
  Kokkos::finalize();
