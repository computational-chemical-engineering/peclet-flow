...
r = run_case("E1", 8)
v = -np.asarray(r["v"]); g = np.asarray(r["gap"]); t = np.asarray(r["t"])
print("nstep=%d dt=%.3f wall=%.0fs peak=%.4f (exp %.3f) final gap=%.3f" % (r["nstep"], r["dt"], r["wall"], v.max(), r["ratio_exp"], g[-1]))
for i in range(0, len(v), max(1, len(v)//20)):
    print("k=%5d gap/D=%7.3f  -v/u*=%8.4f" % (i, g[i], v[i]))
