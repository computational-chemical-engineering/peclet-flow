...
int main(int argc, char** argv) {
  ...
  std::vector<std::uint32_t> bits;
  {
    ...
    Kokkos::deep_copy(cen, hc);
    Kokkos::deep_copy(rad, hr);
    const int G = 8;
    ...
    Kokkos::parallel_for(
        "cap2", NP, KOKKOS_LAMBDA(int i) {
          SdfSpheres<double> sp{cen, rad, M, 2.0};
          auto gr =
              SdfGrid<double>::fromSpacing(Kokkos::View<const float*, peclet::core::MemSpace>(gv),
                                           G, G, G, -1.0, -1.0, -1.0, 0.3, 0.3, 0.3);
