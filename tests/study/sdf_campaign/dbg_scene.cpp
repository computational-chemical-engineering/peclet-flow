...
struct Rng { std::mt19937_64 g{20260829ull};
...
int main() {
  ...
  const int blade = b.addLeaf(kBox, {0.12, 0.02, 0.03}, Transform<double>{Vec3<double>{0.13,0,0}});
    ...
    for (int t = 0; t < 60000; ++t) {
      ...
      if (got != ref) {
        if (bad < 4) {
          ...
          if (bin >= 0) for (int k = gv.offsets[bin]; k < gv.offsets[bin+1]; ++k)
