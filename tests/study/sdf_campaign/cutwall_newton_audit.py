...
def tow(case, DH, frac=None, nstep=700):
    """Prescribed constant fall velocity through the tank; quasi-steady drag vs buoyant weight."""
    rho_f, mu_si, u_inf, ratio_exp, Re = CASES[case]
    frac = ratio_exp if frac is None else frac
    NX = int(np.ceil((100 / 15 * DH + 2 * WALL) / 8) * 8)
    NY = int(np.ceil((160 / 15 * DH + 2 * WALL) / 8) * 8)
    WX = (NX - 100 / 15 * DH) / 2
    WY = (NY - 160 / 15 * DH) / 2
    ...
    s.set_advection(ADV)
    U = frac * USTAR
    Fg = (RATIO - 1.0) * (np.pi / 6) * DH ** 3 * GSTAR
    y = y0; Fs = []
    for k in range(nstep):
        s.set_instance_transform(1, [x0, y, x0])
        s.set_instance_motion(1, lin_vel=[0.0, -U, 0.0])
        s.rebuild_geometry(); s.step()
        FF = np.asarray(s.hydro_force_torque_reaction())[0].astype(float)
        Fs.append((FF[1][1], FF[0][1]))
        y -= U * dt
        if y - (WY + OFF) - DH / 2 < 2.0 * DH:
            break
    Fs = np.array(Fs)
    n4 = max(len(Fs) * 3 // 4, len(Fs) - 200)
    Fsp = Fs[n4:, 0].mean(); Ftk = Fs[n4:, 1].mean()
    print("%s tow adv=%d: F_sphere/W=%.4f  F_tank/W=%.4f  sum/W=%.4f  (W=%.4f steps=%d)"
          % (case, ADV, Fsp / Fg, Ftk / Fg, (Fsp + Ftk) / Fg, Fg, len(Fs)))
