...
def main():
    ...
    for name, cls in (("staggered", pf.Solver), ("collocated", pf.SolverColocated)):
        ...
        ok &= good
        print(f"{name:10s} step OFF {1e3 * off:7.2f} ms  ON {1e3 * onn:7.2f} ms  ratio {ratio:.3f}   "
              f"iters: main OFF {p_off:.1f}, main ON {p_on:.1f}, balanced-force ON {b_on:.1f}   "
