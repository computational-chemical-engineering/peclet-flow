# CLAUDE.md as it stood on 2026-09-08 (the 174 KB version) — archived record

*Archived by QUALITY_PLAN.md §3.H.2 on 2026-09-08, when `CLAUDE.md` was cut to a working
reference. Nothing was deleted: this is the file verbatim, including the whole VoF rung-by-rung
narrative (V0-W3, the phase-change rungs P0-P3g), the collocated attractor campaign summary, and
the WO-F/G/H/I/M findings. It is a **snapshot of its date** — file:line citations, "DEFAULT since"
lines and test counts are not maintained here; the live reference is `../../CLAUDE.md` and the
notes in `../`. Its relative markdown links were re-based onto this directory; the bare `doc/...`
and `src/...` paths in its prose are as written on the day. The environment-variable table it
contains is history too: no environment variable
changes a result any more (QUALITY_PLAN D3, flow `ad917b1`).*
...
- `src/flow_bindings.cpp` - the `flow` nanobind module: `peclet.flow.Solver` (staggered MAC, default) and
  ...
  approximate projection — identical Python API; see [`doc/flow_colocated_plan.md`](../flow_colocated_plan.md))
...
`-DPECLET_FLOW_OPERATOR_DOUBLE`) — not a metric defect (`doc/units_escalation.md` E1).
...
- **What remains: a high coefficient CONTRAST makes the V-cycle preconditioner indefinite.** Both CG
  ...
  default**. Measurements: WO-B/WO-C/WO-H in [`doc/vof_workorders.md`](vof_workorders.md);
  reproduce with `tests/study/vardensity_solver_probe.py --drivers pcg,fcg`.
  **WO-M (2026-08-31) re-ran that dense probe on a `-DPECLET_FLOW_OPERATOR_DOUBLE` hierarchy and the
...
- **Operator STORAGE precision (`MReal`) is a separate, measured axis.** `mac_cutcell_mg.hpp`'s
  ...
  `-DPECLET_FLOW_OPERATOR_DOUBLE` switches both to double. Float rounding breaks the singular row-sum
...
- Validated against Zick & Homsy SC-sphere drag. Design + benchmarks:
  [`doc/flow_multigrid_plan.md`](../flow_multigrid_plan.md).

### Geometric VoF — two-phase flow (rungs V2a + V2b; `src/vof/`)

Campaign plan: [`../docs/VOF_PLAN.md`](../../../docs/archive/VOF_PLAN.md); work orders + findings:
[`doc/vof_workorders.md`](vof_workorders.md) (phase 0),
[`doc/vof_workorders_v2.md`](vof_workorders_v2.md) (V2),
[`doc/vof_workorders_v34.md`](vof_workorders_v34.md) (V3/V4) and
[`doc/vof_workorders_v5.md`](vof_workorders_v5.md) (V5a cut cells, and the open V-BC/V5b/V8
work orders).
...
- **At variable density the equilibrium is approached, not hit.** The semi-implicit momentum
  ...
  figures in a `-DPECLET_FLOW_OPERATOR_DOUBLE` build. At μ = 0 that build reads **6.8e-17 … 8.9e-18 at
...
defect unrelated to VoF — a `-DPECLET_FLOW_OPERATOR_DOUBLE` build measures 1.2e-15, flat across four
