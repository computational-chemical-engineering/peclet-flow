...
\subsection{Modified Matrix Coefficients}
In the linear solver, we construct the modified row coefficients $a'$ and source term $b'$ as follows:
\begin{align}
    a'_c &= \mathcal{D} \, a_c + \mathcal{N}_c \, a_{ghost}\\
    a'_{nb,d} &= \mathcal{D} \, a_{nb,d} + \mathcal{N}_{nb,d} \, a_{ghost} \\
    a'_{nb,\mathrm{other}} &= \mathcal{D} \, a_{nb,\mathrm{other}} \quad \text{(other directions)} \\
    a'_{ghost} &= 0 \\
    b'_c &= \mathcal{D} \, b_c -  \mathcal{N}_{bc} \, u_{bc} \, a_{ghost}
\end{align}
...
We construct a quadratic polynomial that satisfies the boundary conditions $u_{bc,-}$ and $u_{bc,+}$ exactly. The linear system row for cell $c$ is modified to eliminate both neighbors ($a_{nb,-}$ and $a_{nb,+}$):
\begin{equation}
    a'_c u_c = b'_c
\end{equation}
The modified coefficient and source term are derived as:
\begin{align}
    a'_c &= \mathcal{D} \, a_c +  \mathcal{N}_{c,-} \, a_{ghost,-} + \mathcal{N}_{c,+} \, a_{ghost,+} \\
    b'_c &= \mathcal{D} \, b_c - \mathcal{N}_{bc,-,-}  \, u_{bc,-} \,  a_{ghost,-} - \mathcal{N}_{bc,-,+}  \, u_{bc,-} \, a_{ghost,+} \nonumber \\
    & \quad
        - \mathcal{N}_{bc,+,-}  \, u_{bc,+} \, a_{ghost,-}
     - \mathcal{N}_{bc,+,+}  \, u_{bc,+} \,  a_{ghost,+}
\end{align}
Although the expression for $b'_c$ is complex, factors can be pre-computed and combined if the boundary values are time-independent. Furthermore, for stationary objects with no-slip conditions ($u_{bc}=0$), many terms vanish.
...
\begin{table}[h!]
    ...
    \begin{tabular}{|l|c|c|}
        ...
        \hline
        $\mathcal{D}$ & $(\xi_- + \xi_+) \xi_- \xi_+$ & $(\xi_- + \xi_+) (\xi_- \xi_+ - \frac{1}{12})$ \\
        \hline
        $\mathcal{N}_{c,+}$& $(\xi_- + \xi_+) (\xi_- + 1)(\xi_+ - 1)$ & $(\xi_- + \xi_+) [ (\xi_- + 1)(\xi_+ - 1) - \frac{1}{12} ]$ \\
        \hline
        $\mathcal{N}_{bc,+,+}$& $\xi_- (1 + \xi_-)$ & $\xi_- (1 + \xi_-) - \frac{1}{12}$ \\
        \hline
        $\mathcal{N}_{bc,-,+}$ & $\xi_+ (1 - \xi_+)$ & $\xi_+ (1 - \xi_+) + \frac{1}{12}$ \\
        \hline
    \end{tabular}%
...
\end{equation}
The modified coefficients (for the one-sided case) become:
\begin{align}
    a'_c &= \mathcal{D}_\mathrm{rescale} \, a_c + \sum_d \mathcal{N}_{c,d} \, \frac{\mathcal{D}_\mathrm{rescale}}{\mathcal{D}_{d}} \, a_{ghost,d}\\
    a'_{nb,d} &= \mathcal{D}_\mathrm{rescale} \, a_{nb,d} + \sum_d \mathcal{N}_{nb,d}  \, \frac{\mathcal{D}_\mathrm{rescale}}{\mathcal{D}_{d}} \, a_{ghost,d} \\
    a'_{nb,\mathrm{other}} &= \mathcal{D}_\mathrm{rescale} \, a_{nb,\mathrm{other}} \quad \text{(other directions)} \\
    a'_{ghost,d} &= 0 \\
    b'_c &= \mathcal{D}_\mathrm{rescale} \, b_c - \sum_d  \mathcal{N}_{bc,d} \, \frac{\mathcal{D}_\mathrm{rescale}}{\mathcal{D}_d} \, u_{bc,d} \, a_{ghost,d}
\end{align}
The summation runs over all directions where the IBM is applied. For directions where $d \neq d_{min}$, the ratio $|\mathcal{D}_\mathrm{rescale}/\mathcal{D}_d| \le 1$. A singularity ($0/0$) may seemingly arise if $\mathcal{D}_\mathrm{rescale} \approx 0$ and $\mathcal{D}_{d} \approx 0$. This occurs when the IBM interaction point is at (for point values) or very close to (for cell averages) the cell center. In such instances, we define $\mathcal{D}_\mathrm{rescale}/\mathcal{D}_d = 1$. Since the interaction points in all directions are spatially proximate, the boundary values $u_{bc,d}$ are similar, and the boundary condition remains well-posed.
...
To avoid conditional branching during the linear solve, we employ a unified "bake-in" strategy. During initialization, we identify all cells $c$ that require IBM corrections (i.e., cells with at least one solid neighbor). These cells are stored in a single list, $\mathcal{L}_{IBM}$.

For each cell in $\mathcal{L}_{IBM}$, we pre-compute a set of scaling and modification factors for all three spatial directions ($d \in \{x, y, z\}$). Correct handling of the stencil requires modifying the center coefficient, the neighbor coefficients, and the source term.
...
\begin{itemize}
    ...
    \item $B_{val}$: Correction for the source term $b_c$.
\end{itemize}
...
We launch a kernel over the list $\mathcal{L}_{IBM}$ to apply the geometric constraints. This kernel operates without conditional branches by executing the same sequence of floating-point operations for every item.

For a cell $c$, we first scale the center coefficient:
\begin{equation}
   a_c \leftarrow a_c \cdot \mathcal{D}_{rescale}
\end{equation}
Then, we loop over the three spatial directions $d \in \{x, y, z\}$. Let $a_{nb,-}$ and $a_{nb,+}$ denote the coefficients of the "left" (minus) and "right" (plus) neighbors in direction $d$. The update proceeds as follows:
\begin{align}
    % Add neighbors to center
    a_c &\leftarrow a_c + a_{nb,-} \cdot K_{c,-} + a_{nb,+} \cdot K_{c,+} \\
    % Compute temp cross values
    T_- &= a_{nb,-} \cdot X_- \\
    T_+ &= a_{nb,+} \cdot X_+ \\
    % Update neighbors
    a_{nb,-} &\leftarrow a_{nb,-} \cdot M_- + T_+ \\
    a_{nb,+} &\leftarrow a_{nb,+} \cdot M_+ + T_-
\end{align}
Finally, the right-hand side is updated:
\begin{equation}
    b_c \leftarrow b_c \cdot \mathcal{D}_{rescale}.
\end{equation}
The values $a_c$, $a_{nb,\pm}$ and $b_c$ refer to the entries in the specific stencil arrays (e.g., $\mathcal{A}_C$, $\mathcal{A}_W, \mathcal{A}_E$, $\mathcal{B}$) at index $c$. This branchless logic covers all topological cases, including sandwiched cells, by appropriate setting of the pre-computed factors (e.g., setting $M=0$ to zero out a ghost connection). The update uses atomic operations when modifying the center coefficient to handle potential race conditions if multiple directions contribute simultaneously, though in this structured loop approach over directions for a single cell, race conditions on $a_c$ are naturally avoided within the thread. Note that updating neighbors $a_{nb,-}, a_{nb,+}$ theoretically could race if two different item threads update the same neighbor, but since the list iterates over \emph{affected cells} and updates their \emph{own} stencil entries (row-based modification), no race condition exists between threads for the matrix coefficients of cell $c$.
...
To validate the solver's capability in handling complex porous media, we compute the flow through Simple Cubic (SC) arrays of spheres. The drag force exerted on the spheres is quantified using the dimensionless drag factor $K$. Our results are compared with the reference values provided by Zick and Homsy (1982). We observe excellent agreement, reproducing $K \approx 4.95$ for a solid volume fraction $\phi=0.05$ and $K \approx 39.5$ for $\phi=0.50$.
