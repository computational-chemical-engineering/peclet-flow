...
///
/// Composes the ported operators into a Chorin projection step on a staggered MAC grid (periodic
/// box, ghost width 2): explicit Koren-TVD advection (optional) + backward-Euler implicit diffusion
/// (RB-GS) -> divergence -> pressure Poisson (RB-GS + mean removal) -> projection correction.
/// Divided convention (operator (1/dt)I - nu*Lap). Single GPU: the "halo" is a periodic ghost wrap.
/// This is the cfd analog of the packing demStep — it wires the validated kernels into a runnable
/// solver.
...
class FlowReference {
 public:
  ...
  void step() {
    ...
    Kokkos::deep_copy(us_, u_);
    Kokkos::deep_copy(vs_, v_);
    Kokkos::deep_copy(ws_, w_);
    ...
    Kokkos::deep_copy(u_, us_);
    Kokkos::deep_copy(v_, vs_);
    Kokkos::deep_copy(w_, ws_);
  }
  ...
  void buildRhs() {
    SExec space;
    const double id = idiag();
    const bool adv = advect_;
  ...
  void removeMean(F f) {
    ...
    const I3 e = e_;
    Kokkos::parallel_for(
        "peclet::flow::submean", Kokkos::RangePolicy<SExec>(space, 0, ne_),
  ...
  void fillAxis(F f, int axis) {
    ...
    // fill.
    Kokkos::parallel_for(
        "peclet::flow::pfill", MD(space, {0, 0}, {dims[b], dims[c]}),
