...
#ifndef CFD_SDFLOW_KOKKOS_HPP
#define CFD_SDFLOW_KOKKOS_HPP
...
class SdflowKokkos {
 public:
  ...
  void buildRhs() {
    ...
    Kokkos::parallel_for(
        "cfdk::sdflow_rhs", MD(space, {G, G, G}, {e.x - G, e.y - G, e.z - G}),
        KOKKOS_LAMBDA(int x, int y, int z) {
          const long i = (long)x + (long)y * e.x + (long)z * (long)e.x * e.y;
          double au = 0, av = 0, aw = 0;
          if (adv) {
            sadvk::ViewAcc U{SConst(u), e.x, e.y}, V{SConst(v), e.x, e.y}, W{SConst(w), e.x, e.y};
            au = sadvk::advect(0, x, y, z, U, V, W, U);
            av = sadvk::advect(1, x, y, z, U, V, W, V);
            aw = sadvk::advect(2, x, y, z, U, V, W, W);
          }
          bu(i) = id * u(i) + fx - au;
          bv(i) = id * v(i) + fy - av;
          bw(i) = id * w(i) + fz - aw;
  ...
  SumMax reduceInner(SConst f, bool sq) {
    ...
    Kokkos::parallel_reduce(
        "cfdk::sdflow_reduce", Kokkos::RangePolicy<SExec>(space, 0, (long)N * N * N),
  ...
  void fillAxis(F f, int axis) {
    ...
    Kokkos::parallel_for(
        "cfdk::pfill", MD(space, {0, 0}, {dims[b], dims[c]}), KOKKOS_LAMBDA(int p0, int p1) {
