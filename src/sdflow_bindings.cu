...
class Solver {
 public:
  ...
  void set_velocity_mg_volfrac(bool on, double eps, bool res_mask, bool staircase) {
    s_.set_velocity_mg_volfrac(on, eps, res_mask, staircase);
  }
...
PYBIND11_MODULE(sdflow, m) {
  ...
  py::class_<Solver>(m, "Solver")
      ...
      .def("set_velocity_mg_volfrac", &Solver::set_velocity_mg_volfrac, py::arg("on"),
           py::arg("eps") = 0.1, py::arg("res_mask") = true, py::arg("staircase") = true)
