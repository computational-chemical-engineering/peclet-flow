...
class VofBlockSet {
 public:
  ...
  /// Instrument: fence-bounded wall time of the debris/residue pass (all blocks, all calls) while
  /// `timingOn`; `debrisCalls` counts block-passes. Diagnostic only.
  bool timingOn = false;
  double debrisSeconds = 0.0;
  long debrisCalls = 0;
  static double tick_() {
    Kokkos::fence();
    return WyAdvector::wallSeconds();
  }
