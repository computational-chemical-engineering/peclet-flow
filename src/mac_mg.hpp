...
/// mean removal) into a correction-scheme V-cycle for Lap(phi)=d on a periodic staggered grid.
/// Coarse operators are the rediscretized constant-coefficient Laplacian at the level spacing (A =
/// -Lap = (6 phi - sum)/h^2). This replaces the slow plain RB-GS pressure solve in FlowReference.
/// Single GPU (periodic ghost wrap per level). Any Kokkos backend.
#ifndef PECLET_FLOW_MAC_MG_HPP
#define PECLET_FLOW_MAC_MG_HPP
...
class MgPoisson {
 public:
  ...
  void solve(SField phi, SConst d, int nVcycles, int nu1 = 2, int nu2 = 2) {
    ...
    SField f0 = lv_[0].f;
    Kokkos::parallel_for(
        "peclet::flow::mg_negd", Kokkos::RangePolicy<SExec>(space, 0, f0.extent(0)),
        KOKKOS_LAMBDA(std::size_t i) { f0(i) = -d(i); });

    Kokkos::deep_copy(lv_[0].phi, phi);
    for (int v = 0; v < nVcycles; ++v) {
      vcycle(0, nu1, nu2);
      removeMean(lv_[0].phi, lv_[0]);
    }
  ...
  double finestResidualMax() {
    computeResidual(0);
    return maxAbsInner(lv_[0].r, lv_[0]);
  }

  // Public so the Kokkos launch-compiler stubs (and the extended-lambda enclosing methods below)
  // can name these; the V-cycle internals are not meant for external use.
  ...
  void removeMean(SField f, Level& L) {
    ...
    Kokkos::parallel_reduce(
        "peclet::flow::mg_mean", Kokkos::RangePolicy<SExec>(space, 0, (long)N * N * N),
    ...
    Kokkos::parallel_for(
        "peclet::flow::mg_submean", Kokkos::RangePolicy<SExec>(space, 0, f.extent(0)),
  ...
  double maxAbsInner(SField f, Level& L) {
    ...
    double m = 0;
    Kokkos::parallel_reduce(
        "peclet::flow::mg_maxabs", Kokkos::RangePolicy<SExec>(space, 0, (long)N * N * N),
        KOKKOS_LAMBDA(long c, double& acc) {
          const int ix = (int)(c % N), iy = (int)((c / N) % N), iz = (int)(c / ((long)N * N));
